...
public class WxPayApi {
    ...
    public String unifiedOrder(WxRequestParam wxRequestParam) throws PayException, IOException, IllegalAccessException, NoSuchAlgorithmException {
        ...
        if(StringUtil.isEmpty(wxRequestParam.getSpbill_create_ip()))
            wxRequestParam.setSpbill_create_ip(wxConfig.getSpbill_create_ip());
    ...
    public String refund(WxRequestParam wxRequestParam) throws PayException, IOException, IllegalAccessException, NoSuchAlgorithmException, Exception {
        ...
        wxRequestParam.setMch_id(wxConfig.getMch_id());//商户号
        wxRequestParam.setNonce_str(StringUtil.getRandomStr(32));
        if(!StringUtil.isEmpty(wxRequestParam.getTransaction_id())){
            wxRequestParam.setOut_trade_no(null);
        }
        sign(wxRequestParam);
        String xml = toXml(wxRequestParam);
        log.info(certManager.getWxCert(wxConfig.getCert_path())+"tk---------------------------xml"+xml);
        return HttpClientManager.doHttpsPost(PayConstant.WX_REFUND_URL,xml,null,certManager.getWxCert(wxConfig.getCert_path()), wxConfig.getMch_id());
...
//            if(map.get("sign").equals(sign))
    ...
    public WxConfig toConfigSiteId(Integer siteId){
        ...
        if("NO".equals(fw)||payMap==null)return wxConfig;
        WxConfig wxConfig=new WxConfig(payMap.get("wx_app_id").toString(), payMap.get("wx_appsecret").toString(), payMap.get("wx_mch_id").toString(), "", payMap.get("wx_spbill_create_ip").toString(), payMap.get("wx_key").toString(), payMap.get("wx_cert_path").toString(), "");
        return wxConfig;
    }
}
